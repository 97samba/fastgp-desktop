...
const Creation = () => {
  ...
  const [pageMode, setpageMode] = useState(queryParams.get("mode") || "create");
        ...
        if (currentUser?.uid) {
          if (
            ...
            if (history.location.state?.publication) {
              ...
              queryParams.get("id")
                ? (model = await getAFlight(queryParams.get("id")))
                : history.push("/");
            }
            ...
            setstate({
              ...
              contributionPaymentMethod: model.contributionPaymentMethod,
