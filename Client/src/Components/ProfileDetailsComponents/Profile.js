...
import { FaPlaneDeparture, FaStar, FaUserAlt, FaUserEdit, FaUserPlus } from "react-icons/fa";
...
import {
  FollowGP,
  getFeedbacksFromGPId,
  getUserRecentFlights,
  UnFollowGP,
} from "../../firebase/db";
          ...
          <Box>
            {flights.length > 0 ? (
              ...
              <Paper
                ...
                elevation={0}
              >
                <Typography color="GrayText">Pas d'annonces.</Typography>
              </Paper>
            )}
          </Box>
        )}
        ...
        {currentUser?.uid === id ? (
          ...
          <Box>
            {user?.followers.length > 0 && user.followers.includes(currentUser?.uid) ? (
              <LoadingButton loading={following} fullWidth variant="outlined" onClick={unFollow}>
                Se désabonner
              </LoadingButton>
            ) : (
              <LoadingButton
